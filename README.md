...
DSH 的 Windows 桌面启动器(Tauri 2 + Vanilla TS)。打开应用即自动启动 DeepSeek Harness 的 Web 服务并把其界面内嵌在无边框窗口里,同时负责 dsh 的版本检查与一键升级。

## 功能

- **极致轻量**:安装包仅 **3.27 MB**,秒下载、秒安装,Windows 原生体验。
- **一键启动**:启动时自动启动 DeepSeek Harness 服务(通过 `dsh` 的 `web` 子命令),服务就绪后在窗口内嵌显示 Web 界面(`http://127.0.0.1:3080`);若服务已在运行(包括外部启动的),直接挂载,绝不重复启动。
...
- **关闭不终止服务**:关闭窗口即退出应用,但 DeepSeek Harness 服务留在后台继续运行(下次打开时自动复用);需要停止时在界面点击「停止」;单实例运行,重复启动只聚焦已有窗口。
- **服务管理**:状态条提供「重启服务」「停止服务」按钮,对任何由本机 node 进程承载的 DeepSeek Harness 服务生效——包括上次会话遗留、手动启动的实例(按 3080 端口精确定位监听进程并终止其进程树,不误伤其他 node 程序)。仅当端口由非 node 进程持有(如 WSL 中继 `wslrelay.exe`,即服务运行在 WSL 内)时不受桌面端控制:点击按钮会给出真实持有进程与指引,由你在原环境中操作,桌面端每 5 秒探测、服务恢复后自动重新挂载。
...
```
├─ src/                    # 前端(原生 TS,无框架)
│  ├─ main.ts              # 状态机、事件监听、升级/重启/停止流程
│  ├─ styles.css           # 主题变量与界面样式
│  └─ assets/logo.svg      # 应用内图标(与根目录 icon.svg 同源)
├─ src-tauri/
│  ├─ src/
│  │  ├─ dsh.rs            # DeepSeek Harness 进程管理、健康探测、状态机
...
| 环境变量 | 说明 |
|----------|------|
| `DSH_DESKTOP_PORT` | 覆盖 DeepSeek Harness 服务端口(默认 3080,会以 `dsh web --port <N>` 传递) |
...
- **关窗后 DeepSeek Harness 仍在后台运行(预期行为)**:关闭窗口只退出桌面端,服务会继续在后台运行,下次打开桌面端时自动复用;若需彻底停止,在界面点击「停止」或执行 `taskkill /IM node.exe /F`(仅当没有其他 Node 程序)。
